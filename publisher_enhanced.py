...
class VibeLookupClient:
    ...
    def lookup_vibe_ids(self, vibe_slugs: List[str]) -> Tuple[List[int], List[str]]:
        ...
        try:
            ...
            logger.error(f"Vibe lookup failed: {response.status_code} - {response.text}")
            return [], vibe_slugs
            
        except Exception as e:
            logger.error(f"Error looking up vibes: {e}")
            return [], vibe_slugs
...
class EnhancedPublisher:
    ...
    def _extract_vibe_slugs(self, data: Dict) -> List[str]:
        """Extract all unique vibe slugs from the restaurant data"""
        vibe_slugs = []
        
        # Check if vibes are at the list level
        if 'vibes' in data and isinstance(data['vibes'], list):
            vibe_slugs.extend(data['vibes'])
        
        # Extract vibes from each restaurant
        for restaurant in data.get('restaurants', []):
            if 'vibes' in restaurant and isinstance(restaurant['vibes'], list):
                vibe_slugs.extend(restaurant['vibes'])
        
        # Also check for single vibe field
        if 'vibe' in data and data['vibe']:
            vibe_slugs.append(data['vibe'])
        
        # Return unique slugs
        return list(set(filter(None, vibe_slugs)))
    
    def _prepare_master_critic_data(self, data: Dict, vibe_ids: List[int]) -> Dict:
        """Prepare data for Master Critic post type"""
